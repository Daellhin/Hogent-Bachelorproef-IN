...
import { mod, modRad } from '../../utils/mathUtils';
...
export function useHeading(acc, mag, gyr, offset?) {
  // Constractor
  const acc_inv = object_sign_inversion(acc);
 ...
 */
export function useCompassHeading(mag: { x: number; y: number; z: number;}, offset?: number) {
  let angle = 0;
  if (mag) {
    let { x, y, z } = mag;
    if (Math.atan2(y, x) >= 0) {
  ...
  if (offset) {
    return modRad(angle + offset);
  }
  return angle;
}

export function useStepLength(acc, mag, gyr, steplengthScaler?, offset?) {
  // Custom Hooks
  const [accStep, accEvent] = useAccStep(acc, mag, gyr);
  const heading = useHeading(acc, mag, gyr, offset);
  ...
  return [ret.stepLength * steplengthScaler, ret.headingStep, heading];
}
